...
(function () {
    ...
    {},
    {
      ...
      regexLiteral: function (rx /* must start with ^ */) {
        ...
        return FP.Parsers.Parser(function (input) {
          ...
          if (m !== null) {
            return FP.Parsers.Success(
              m[0],
              input.drop(start + m[0].length - offset)
            );
          } else {
            if (start === source.length) {
              found = "end of source";
      ...
      eos: function () {
        return FP.Parsers.Parser(function (input) {
          ...
          if (source.length === start) {
            ...
            return FP.Parsers.Failure(
              "end of source expected but `" + source.charAt(start) + "' found",
              input
            );
          }
        });
      ...
      chainr1: function (/*lazy*/ p, /*lazy*/ q, combine, first) {
        return p()
          .and(this.rep(q().and(p)))
          .to(function (res) {
            return FP.List.Cons(
              FP.Parsers.Pair(combine, res.head),
              res.tail
            ).foldRight(first, function (fa, b) {
  ...
  FP.Parsers.NoSuccess = FP.Parsers.ParseResult.Subclass({
    ...
    _setLastNoSuccess: function () {
      ...
      if (
        context.lastNoSuccess === undefined ||
        !this.next.pos().isLessThan(context.lastNoSuccess.next.pos())
      ) {
...
MathJax.Extension.xypic = {
  ...
  constants: {
    ...
    unsupportedBrowserErrorMessage:
      "Unsupported Browser. Please open with Firefox/Safari/Chrome/Opera",
  },
  signalHandler: {
    signals: [],
    hookedSignals: [],
...
MathJax.Extension.xypic.signalHandler.chainSignal(
  "Device-Independent Xy-pic Require",
  ["HTML-CSS Xy-pic Config Ready"]
);
MathJax.Extension.xypic.signalHandler.chainSignal(
  "Device-Independent Xy-pic Require",
  ["SVG Xy-pic Config Ready"]
);
...
MathJax.Extension.xypic.signalHandler.chainSignal("HTML-CSS Xy-pic Require", [
  "HTML-CSS Xy-pic Config Ready",
  "Device-Independent Xy-pic Ready",
]);
...
MathJax.Hub.Register.StartupHook("TeX Xy-pic Require", function () {
  ...
  AST.ObjectBox.Curve = AST.ObjectBox.Subclass({
    ...
    toString: function () {
      return (
        "\\curve" +
        this.modifiers.mkString("") +
        "{" +
        this.objects.mkString(" ") +
        " " +
        this.poslist.mkString("&") +
        "}"
      );
    },
  });
  ...
  AST.Command.Ar.Form.BuildArrow = AST.Command.Ar.Form.Subclass({
    ...
    toString: function () {
      return (
        ...
        this.variant +
        "{" +
        this.tailTip.toString() +
        ", " +
        this.stemConn.toString() +
        ", " +
        this.headTip.toString() +
        "}"
      );
    },
  });
  ...
  AST.Command.Twocell.Twocell = AST.Command.Twocell.Hops2cell.Subclass({
    toString: function () {
      return (
        "\\xtwocell[" + this.hops + "]" + this.maybeDisplace.getOrElse("{}")
      );
    },
  });
  //           |   '\' /[lrud]+/ 'uppertwocell'
  ...
  AST.Command.Twocell.Switch.ChangeCurveObject = MathJax.Object.Subclass({
    ...
    toString: function () {
      return (
        ...
        "{" +
        this.spacer +
        (this.maybeObject.isDefined ? "~**" + this.maybeObject.get : "") +
        "}"
      );
    },
  });
  ...
  var p = FP.Parsers.Subclass({
    ...
    xymatrixbox: memo(function () {
      return p.xymatrix().to(function (m) {
        return AST.PosDecor(
          AST.Pos.Coord(AST.Coord.C(), FP.List.empty),
          AST.Decor(FP.List.empty.append(m))
        );
      });
    }),
    ...
    nonemptyCoord: memo(function () {
      return or(
        ...
        lit("[")
          .andr(
            fun(
              opt(lit('"').andr(p.id).andl(felem('"'))).to(function (id) {
                return id.getOrElse("");
              })
            )
          )
          ...
          .andl(flit("]"))
          .to(function (prc) {
            return AST.Coord.DeltaRowColumn(
              prc.head.head,
              prc.head.tail,
              prc.tail
            );
      ...
      return or(
        ...
        function () {
          return p
            ...
            .and(
              fun(
                FP.Parsers.opt(fun(lit("(").andr(p.factor).andl(flit(")")))).to(
                  function (f) {
    ...
    unit: memo(
      fun(
        regexLit(/^(em|ex|px|pt|pc|in|cm|mm|mu)/).to(function (d) {
          return d;
        })
      )
    ),
    ...
    txt: memo(function () {
      return lit("\\txt")
        .andr(p.txtWidth)
        .and(fun(regex(/^(\\[a-zA-Z@][a-zA-Z0-9@]+)?/)))
        .andl(flit("{"))
        .and(p.text)
        .andl(flit("}"))
        .to(function (wst) {
    ...
    nonemptyCir: memo(function () {
      return p
        ...
        .and(p.diag)
        .to(function (dod) {
          return AST.ObjectBox.Cir.Cir.Segment(
            dod.head.head,
            dod.head.tail,
            dod.tail
          );
    ...
    arrowForm: memo(function () {
      return or(
        ...
        lit("@")
          .andr(flit("/"))
          .andr(p.direction)
          .and(fun(opt(p.looseDimen)))
          .andl(flit("/"))
          .to(function (dd) {
            return AST.Command.Ar.Form.CurveArrow(
              dd.head,
              dd.tail.getOrElse(".5pc")
            );
          }),
        lit("@")
          .andr(flit("("))
        ...
        lit("@")
          .andr(fun(regex(/^([\^_0123])/).opt()))
          .and(fun(opt(p.tipConnTip)))
          .to(function (vtct) {
            var variant = vtct.head.getOrElse("");
            if (vtct.tail.isDefined) {
              var tct = vtct.tail.get;
              return AST.Command.Ar.Form.BuildArrow(
                variant,
                tct.tail,
                tct.stem,
                tct.head
    ...
    tipConnTip: memo(function () {
      return lit("{")
        .andr(fun(opt(p.nonemptyTip)))
        .and(fun(opt(p.nonemptyConn)))
        .and(fun(opt(p.nonemptyTip)))
        .andl(flit("}"))
        .to(function (pcp) {
    ...
    path: memo(function () {
      return p
        .path2(FP.List.empty /* initial failure continuation */)
        .to(function (ps) {
    ...
    segment: memo(function () {
      return p
        ...
        .and(p.labels)
        .to(function (psl) {
          return AST.Command.Path.Segment(
            psl.head.head,
            psl.head.tail,
            psl.tail
          );
    ...
    label: memo(function () {
      return or(
        seq("^", p.anchor, p.it, p.alias).to(function (aia) {
          return AST.Command.Path.Label.Above(
            AST.Pos.Place(aia.head.head.tail),
            aia.head.tail,
            aia.tail
          );
        }),
        seq("_", p.anchor, p.it, p.alias).to(function (aia) {
          return AST.Command.Path.Label.Below(
            AST.Pos.Place(aia.head.head.tail),
            aia.head.tail,
            aia.tail
          );
        ...
        seq("|", p.anchor, p.it, p.alias).to(function (aia) {
          return AST.Command.Path.Label.At(
            AST.Pos.Place(aia.head.head.tail),
            aia.head.tail,
            aia.tail
          );
    ...
    anchor: memo(function () {
      return or(
        lit("-")
          .andr(p.anchor)
          .to(function (a) {
            return AST.Place(1, 1, AST.Place.Factor(0.5), undefined).compound(
              a
            );
          }),
        p.place
          ...
          or(
            ...
            lit("@!")
              ...
              .to(function (cd) {
                ...
                switch (cd.head) {
                  case "R":
                    return AST.Command.Xymatrix.Setup.PretendEntrySize.Height(
                      dimen
                    );
                  case "C":
                    return AST.Command.Xymatrix.Setup.PretendEntrySize.Width(
                      dimen
                    );
                  default:
                    return AST.Command.Xymatrix.Setup.PretendEntrySize.HeightAndWidth(
                      dimen
                    );
                }
              }),
            lit("@!").andr(
              fun(
                or(
                  elem("R").to(function () {
                    return AST.Command.Xymatrix.Setup.FixGrid.Row();
                  }),
                  elem("C").to(function () {
                    return AST.Command.Xymatrix.Setup.FixGrid.Column();
                  })
                )
                  .opt()
                  .to(function (rc) {
                    return rc.getOrElse(
                      AST.Command.Xymatrix.Setup.FixGrid.RowAndColumn()
                    );
                  })
              )
            ),
            lit("@")
              ...
              .to(function (cod) {
                ...
                switch (cod.head.head) {
                  case "M":
                    return AST.Command.Xymatrix.Setup.AdjustEntrySize.Margin(
                      addop,
                      dimen
                    );
                  case "W":
                    return AST.Command.Xymatrix.Setup.AdjustEntrySize.Width(
                      addop,
                      dimen
                    );
                  case "H":
                    return AST.Command.Xymatrix.Setup.AdjustEntrySize.Height(
                      addop,
                      dimen
                    );
                  case "L":
                    return AST.Command.Xymatrix.Setup.AdjustLabelSep(
                      addop,
                      dimen
                    );
                }
            ...
            lit("@*")
              ...
              .and(p.size)
              .to(function (os) {
                return AST.Command.Xymatrix.Setup.AddModifier(
                  AST.Modifier.AddOp(os.head, os.tail)
                );
              }),
            lit("@")
              .andr(
              ...
              .to(function (cod) {
                ...
                switch (cod.head.head) {
                  case "R":
                    return AST.Command.Xymatrix.Setup.ChangeSpacing.Row(
                      addop,
                      dimen
                    );
                  case "C":
                    return AST.Command.Xymatrix.Setup.ChangeSpacing.Column(
                      addop,
                      dimen
                    );
                  default:
                    return AST.Command.Xymatrix.Setup.ChangeSpacing.RowAndColumn(
                      addop,
                      dimen
                    );
                }
    ...
    entry: memo(function () {
      return or(
        ...
        p
          .entryModifier()
          .rep()
          .and(p.looseObjectbox)
          .and(p.decor)
          .to(function (mopd) {
            var modifiers = mopd.head.head.foldLeft(
              FP.List.empty,
              function (tmpMs, ms) {
    ...
    twocell: memo(function () {
      return or(
        ...
        or(
          ...
          .andl(flit("}"))
          .to(function (cht) {
            var textObject = AST.Object(
              FP.List.empty,
              p.toMath("\\labelstyle " + cht.tail)
            );
      ...
      return or(
        ...
        regexLit(/^(~[`'])/)
          ...
          .to(function (wo) {
            ...
            return AST.Command.Twocell.Switch.ChangeHeadTailObject(
              what,
              wo.tail
            );
          }),
    ...
    twocellArrow: memo(function () {
      return or(
        lit("{")
          .andr(fun(regexLit(/^([\^_=`'"!]|\\omit)/)))
          .and(p.twocellLabelEntry)
          .andl(flit("}"))
          .to(function (te) {
        ...
        success("no arrow label").to(function () {
          ...
          return AST.Command.Twocell.Arrow.WithOrientation(
            "",
            AST.Object(FP.List.empty, p.toMath("\\twocellstyle{}"))
          );
        })
      );
    }),
    ...
    newdir: memo(function () {
      return lit("{")
        ...
        .andl(felem("}"))
        .andl(flit("{"))
        .and(p.compositeObject)
        .andl(flit("}"))
        .to(function (mc) {
          return AST.Command.Newdir(
            mc.head,
            AST.ObjectBox.CompositeObject(mc.tail)
          );
        });
    }),
      ...
      return lit("\\xyimport")
        ...
        .to(function (whog) {
          ...
          if (graphics.isIncludegraphics !== undefined) {
            ...
            return AST.Pos.Xyimport.TeXCommand(
              w,
              h,
              xOffset,
              yOffset,
    ...
    includegraphics: memo(function () {
      return lit("[")
        .andr(fun(opt(p.includegraphicsAttrList)))
        .andl(flit("]"))
        .andl(flit("{"))
        .and(fun(regexLit(/^[^\s{}]+/)))
        .andl(flit("}"))
    ...
    includegraphicsAttrList: memo(function () {
      return p
        .includegraphicsAttr()
        .and(fun(rep(lit(",").andr(p.includegraphicsAttr))))
        .to(function (aas) {
  ...
  xypic.ParseError = MathJax.Object.Subclass({
    ...
    toMML: function () {
      ...
      return MML.merror(
        MML.mtext('parse error at or near "' + lineContents + '"')
      );
      /*
      var col = pos.column();
  ...
  MathJax.Hub.Browser.Select({
    ...
    MSIE: function (browser) {
      if (
        MathJax.Hub.Browser.versionAtLeast("9.0") &&
  ...
  function () {
    ...
    xypic.ModifierRepository = MathJax.Object.Subclass(
      {
        ...
        get: function (shapeName) {
          var modifier =
            xypic.ModifierRepository.embeddedModifierMap[shapeName];
    ...
    xypic.Graphics.SVG.World = xypic.Graphics.SVG.Subclass({
      ...
      extendBoundingBox: function (boundingBox) {
        this.boundingBox = xypic.Frame.combineRect(
          this.boundingBox,
          boundingBox
        );
      },
      ...
      rotateDegree: function (degree) {
        return xypic.Graphics.SVG.Transform(
          this.transform.append(
            xypic.Graphics.SVG.Transform.Rotate((degree / 180) * Math.PI)
          )
        );
      },
    ...
    xypic.Graphics.SVG.Group = xypic.Graphics.SVG.Subclass({
      Init: function (parent, transform) {
        this.parent = parent;
        this.drawArea = parent.createSVGElement(
          "g",
    ...
    xypic.Graphics.Augment(
      {},
      {
        createSVG: function (height, depth, width, strokeWidth, color, def) {
          return xypic.Graphics.SVG.World(
            height,
            depth,
            width,
            strokeWidth,
      ...
      },
      {
        combineRect: function (frame1, frame2) {
          if (frame1 === undefined) {
            ...
            var l = -(
              Math.min(frame1.x - frame1.l, frame2.x - frame2.l) - frame1.x
            );
            var r =
              Math.max(frame1.x + frame1.r, frame2.x + frame2.r) - frame1.x;
            var d = -(
              Math.min(frame1.y - frame1.d, frame2.y - frame2.d) - frame1.y
            );
            var u =
              Math.max(frame1.y + frame1.u, frame2.y + frame2.u) - frame1.y;
    ...
    xypic.Frame.Rect = xypic.Frame.Subclass({
      ...
      proportionalEdgePoint: function (x, y) {
        ...
        var dy = y - this.y;
        if (
          Math.abs(dx) < AST.xypic.machinePrecision &&
    ...
    xypic.Frame.Ellipse = xypic.Frame.Subclass({
      ...
      edgePoint: function (x, y) {
        ...
        if (this.isPerfectCircle()) {
          ...
          if (
            sign(xp0 - cx) === sign(x - cx) &&
            sign(yp0 - cy) === sign(y - cy)
          ) {
    ...
    xypic.Shape.TranslateShape = xypic.Shape.Subclass({
      ...
      draw: function (svg) {
        var g = svg.createGroup(
          svg.transformBuilder().translate(this.dx, this.dy)
        );
        this.shape.draw(g);
      },
      getBoundingBox: function () {
    ...
    xypic.Shape.CompositeShape = xypic.Shape.Subclass({
      ...
      getBoundingBox: function () {
        return xypic.Frame.combineRect(
          this.foregroundShape.getBoundingBox(),
          this.backgroundShape.getBoundingBox()
        );
      },
      toString: function () {
        return (
          "(" +
          this.foregroundShape.toString() +
          ", " +
          this.backgroundShape.toString() +
          ")"
        );
      },
    });
      ...
      draw: function (svg) {
        ...
        if (this.isDoubled) {
          def = {
            ...
            width: xypic.em2px(
              this.left + this.right - 2 * AST.xypic.thickness
            ),
    ...
    xypic.Shape.LeftBrace = xypic.Shape.Subclass({
      ...
      draw: function (svg) {
        ...
        var down =
          Math.max(0.759375 + 0.660375, (this.down / scale) * 1.125) - 0.660375;
        var up =
          -Math.max(0.759375 + 0.660375, (this.up / scale) * 1.125) + 0.660375;
        ...
        d =
          ...
          xypic.em2px(down) +
          "T" +
          xypic.em2px(-0.068625) +
          " " +
          xypic.em2px(0.07875 + down) +
          "Q" +
          xypic.em2px(-0.068625) +
          " " +
          xypic.em2px(0.190125 + down) +
          " " +
          xypic.em2px(-0.0585) +
          " " +
          xypic.em2px(0.250875 + down) +
          "T" +
          xypic.em2px(-0.01125) +
          " " +
          xypic.em2px(0.387 + down) +
          "Q" +
          xypic.em2px(0.07425) +
          " " +
          xypic.em2px(0.55575 + down) +
          " " +
          xypic.em2px(0.2475) +
          " " +
          xypic.em2px(0.6525 + down) +
          "L" +
          xypic.em2px(0.262125) +
          " " +
          xypic.em2px(0.660375 + down) +
          "L" +
          xypic.em2px(0.3015) +
          " " +
          xypic.em2px(0.660375 + down) +
          ...
          xypic.em2px(0.30825) +
          " " +
          xypic.em2px(0.653625 + down) +
          "V" +
          xypic.em2px(0.622125 + down) +
          "Q" +
          xypic.em2px(0.30825) +
          " " +
          xypic.em2px(0.60975 + down) +
          " " +
          xypic.em2px(0.2925) +
          " " +
          xypic.em2px(0.60075 + down) +
          "Q" +
          xypic.em2px(0.205875) +
          " " +
          xypic.em2px(0.541125 + down) +
          " " +
          xypic.em2px(0.149625) +
          " " +
          xypic.em2px(0.44775 + down) +
          "T" +
          xypic.em2px(0.07425) +
          " " +
          xypic.em2px(0.239625 + down) +
          "Q" +
          xypic.em2px(0.07425) +
          " " +
          xypic.em2px(0.2385 + down) +
          " " +
          xypic.em2px(0.073125) +
          " " +
          xypic.em2px(0.235125 + down) +
          "Q" +
          xypic.em2px(0.068625) +
          " " +
          xypic.em2px(0.203625 + down) +
          " " +
          xypic.em2px(0.0675) +
          " " +
          xypic.em2px(0.041625 + down) +
          "L" +
          xypic.em2px(0.0675) +
          " " +
          xypic.em2px(0.75825) +
          ...
          xypic.em2px(0.0675) +
          " " +
          xypic.em2px(0.496125) +
          " " +
          xypic.em2px(0.066375) +
          " " +
          xypic.em2px(0.486) +
          "Q" +
          xypic.em2px(0.05625) +
          " " +
          xypic.em2px(0.336375) +
          " " +
          xypic.em2px(-0.021375) +
          " " +
          xypic.em2px(0.212625) +
          "T" +
          xypic.em2px(-0.226125) +
          " " +
          xypic.em2px(0.010125) +
          "L" +
          xypic.em2px(-0.241875) +
          " 0" +
          "L" +
          xypic.em2px(-0.226125) +
          " " +
          xypic.em2px(-0.010125) +
          "Q" +
          xypic.em2px(-0.106875) +
          " " +
          xypic.em2px(-0.084375) +
          " " +
          xypic.em2px(-0.025875) +
          " " +
          xypic.em2px(-0.207) +
          "T" +
          xypic.em2px(0.066375) +
          ...
          xypic.em2px(0.0675) +
          " " +
          xypic.em2px(-0.041625 + up) +
          "Q" +
          xypic.em2px(0.068625) +
          " " +
          xypic.em2px(-0.203625 + up) +
          " " +
          xypic.em2px(0.073125) +
          " " +
          xypic.em2px(-0.235125 + up) +
          "Q" +
          xypic.em2px(0.07425) +
          " " +
          xypic.em2px(-0.2385 + up) +
          " " +
          xypic.em2px(0.07425) +
          " " +
          xypic.em2px(-0.239625 + up) +
          "Q" +
          xypic.em2px(0.093375) +
          " " +
          xypic.em2px(-0.354375 + up) +
          " " +
          xypic.em2px(0.149625) +
          " " +
          xypic.em2px(-0.44775 + up) +
          "T" +
          xypic.em2px(0.2925) +
          " " +
          xypic.em2px(-0.60075 + up) +
          "Q" +
          xypic.em2px(0.30825) +
          " " +
          xypic.em2px(-0.60975 + up) +
          " " +
          xypic.em2px(0.30825) +
          " " +
          xypic.em2px(-0.622125 + up) +
          "L" +
          xypic.em2px(0.30825) +
          " " +
          xypic.em2px(-0.653625 + up) +
          "L" +
          xypic.em2px(0.3015) +
          " " +
          xypic.em2px(-0.660375 + up) +
          "L" +
          xypic.em2px(0.262125) +
          " " +
          xypic.em2px(-0.660375 + up) +
          ...
          xypic.em2px(0.2475) +
          " " +
          xypic.em2px(-0.6525 + up) +
          "Q" +
          xypic.em2px(0.07425) +
          " " +
          xypic.em2px(-0.55575 + up) +
          " " +
          xypic.em2px(-0.01125) +
          " " +
          xypic.em2px(-0.387 + up) +
          "Q" +
          xypic.em2px(-0.048375) +
          " " +
          xypic.em2px(-0.311625 + up) +
          " " +
          xypic.em2px(-0.0585) +
          " " +
          xypic.em2px(-0.250875 + up) +
          "T" +
          xypic.em2px(-0.068625) +
          " " +
          xypic.em2px(-0.07875 + up) +
          "Q" +
          xypic.em2px(-0.0675) +
          " " +
          xypic.em2px(up) +
          ...
          xypic.em2px(-0.27) +
          "Q" +
          xypic.em2px(-0.133875) +
          " " +
          xypic.em2px(-0.2205) +
          " " +
          xypic.em2px(-0.160875) +
          " " +
          xypic.em2px(-0.17775) +
          "T" +
          xypic.em2px(-0.212625) +
          ...
          xypic.em2px(-0.06075) +
          "T" +
          xypic.em2px(-0.293625) +
          " " +
          xypic.em2px(-0.0315) +
          "T" +
          xypic.em2px(-0.307125) +
          " " +
          xypic.em2px(-0.02025) +
          "Q" +
          xypic.em2px(-0.30825) +
          " " +
          xypic.em2px(-0.019125) +
          " " +
          xypic.em2px(-0.30825) +
          " 0" +
          "T" +
          ...
          xypic.em2px(0.021375) +
          " " +
          xypic.em2px(-0.284625) +
          " " +
          xypic.em2px(0.03825) +
          "T" +
          xypic.em2px(-0.2295) +
          " " +
          xypic.em2px(0.091125) +
          "T" +
          xypic.em2px(-0.162) +
          " " +
          xypic.em2px(0.176625) +
          "T" +
          xypic.em2px(-0.10125) +
          " " +
          xypic.em2px(0.30825) +
      ...
      getBoundingBox: function () {
        ...
        return xypic.Frame.Rect(this.x, this.y, {
          l: 0.274 * scale,
          r: 0.274 * scale,
          u: Math.max(((0.759375 + 0.660375) * scale) / 1.125, this.up),
          d: Math.max(((0.759375 + 0.660375) * scale) / 1.125, this.down),
    ...
    xypic.Shape.LeftParenthesis = xypic.Shape.Subclass({
      ...
      draw: function (svg) {
        var scale = xypic.oneem;
        var down =
          Math.max(0.660375, (this.height / 2 / scale) * 1.125) - 0.660375;
        var up = -down;
    ...
    xypic.Shape.GT2ArrowheadShape = xypic.Shape.ArrowheadShape.Subclass({
      ...
      drawDelegate: function (svg) {
        ...
        gu.createSVGElement("path", {
          d:
            "M0,0 Q" +
            xypic.em2px(-0.222 * scale) +
            "," +
            xypic.em2px(-0.02 * scale) +
            " " +
            xypic.em2px(-0.489 * scale) +
            "," +
    ...
    xypic.Shape.UpperColumnArrowheadShape = xypic.Shape.ArrowheadShape.Subclass(
      {
        ...
        getBox: function () {
          return { l: 0, r: 0, u: AST.xypic.lineElementLength, d: 0 };
        },
        drawDelegate: function (svg) {
    ...
    xypic.Shape.LowerColumnArrowheadShape = xypic.Shape.ArrowheadShape.Subclass(
      {
        ...
        getBox: function () {
          return { l: 0, r: 0, u: 0, d: AST.xypic.lineElementLength };
        },
        drawDelegate: function (svg) {
          var l = xypic.em2px(AST.xypic.lineElementLength);
          svg.createSVGElement("line", {
    ...
    xypic.Shape.Column2ArrowheadShape = xypic.Shape.ArrowheadShape.Subclass({
      ...
      drawDelegate: function (svg) {
        var l = xypic.em2px(
          0.5 * (AST.xypic.lineElementLength + AST.xypic.thickness)
        );
        svg.createSVGElement("line", {
          x1: 0,
          y1: l,
    ...
    xypic.Shape.LineArrowheadShape = xypic.Shape.ArrowheadShape.Subclass({
      ...
      getBox: function () {
        return { l: 0, r: AST.xypic.lineElementLength, u: 0, d: 0 };
      },
      drawDelegate: function (svg) {
        var lineLen = xypic.em2px(AST.xypic.lineElementLength);
        svg.createSVGElement("line", {
    ...
    xypic.Shape.Dot3ArrowheadShape = xypic.Shape.ArrowheadShape.Subclass({
      ...
      getBox: function () {
        return { l: 0, r: 0, u: AST.xypic.thickness, d: AST.xypic.thickness };
      },
      drawDelegate: function (svg) {
        var scale = xypic.oneem;
        var vshift = xypic.em2px(AST.xypic.thickness);
      ...
      drawDelegate: function (svg) {
        ...
        svg.createSVGElement("path", {
          d:
            ...
            -hshift +
            ",0 Q" +
            (xypic.em2px(-0.222 * scale) - hshift) +
            "," +
            xypic.em2px(0.02 * scale) +
            " " +
    ...
    xypic.Shape.GTGT2ArrowheadShape = xypic.Shape.ArrowheadShape.Subclass({
      ...
      drawDelegate: function (svg) {
        ...
        var gu2 = svg.createGroup(
          svg
            .transformBuilder()
            .translate(-2 * t, 0)
            .rotateDegree(-10)
        );
        var gd2 = svg.createGroup(
          svg
            .transformBuilder()
            .translate(-2 * t, 0)
            .rotateDegree(10)
    ...
    xypic.Shape.LTLT2ArrowheadShape = xypic.Shape.ArrowheadShape.Subclass({
      ...
      drawDelegate: function (svg) {
        var scale = xypic.oneem;
        var t = AST.xypic.thickness;
        var gu1 = svg.createGroup(
          svg
            .transformBuilder()
            .translate(2 * t, 0)
            .rotateDegree(10)
        );
        var gd1 = svg.createGroup(
          svg
            .transformBuilder()
            .translate(2 * t, 0)
            .rotateDegree(-10)
    ...
    xypic.Shape.LineGT2ArrowheadShape = xypic.Shape.ArrowheadShape.Subclass({
      ...
      drawDelegate: function (svg) {
        ...
        var gu = svg.createGroup(
          svg.transformBuilder().translate(halfLen, 0).rotateDegree(-10)
        );
        var gd = svg.createGroup(
          svg.transformBuilder().translate(halfLen, 0).rotateDegree(10)
        );
        gu.createSVGElement("path", {
          d:
    ...
    xypic.Shape.CurveShape = xypic.Shape.Subclass({
      ...
      toString: function () {
        return (
          ...
          this.curve +
          ", objectForDrop:" +
          (this.objectForDrop !== undefined
            ? this.objectForDrop.toString()
            : "null") +
          ", objectForConnect:" +
          (this.objectForConnect !== undefined
            ? this.objectForConnect.toString()
            : "null") +
          "]"
        );
      },
    ...
    xypic.Curve = MathJax.Object.Subclass(
      {
        ...
        drawParallelCurve: function (svg, vshift) {
          var i,
            n = this.countOfSegments() * AST.xypic.interpolationResolution;
          ...
          xypic.Curve.CubicBeziers.interpolation(ts, x1s, y1s).drawPrimitive(
            svg,
            "none"
          );
          xypic.Curve.CubicBeziers.interpolation(ts, x2s, y2s).drawPrimitive(
            svg,
            "none"
          );
        },
            ...
            switch (variant) {
              case "3":
                ...
                for (var i = 0; i < n; i++) {
                  ...
                  d1 +=
                    " Q" +
                    xypic.em2px(p.x + 2 * nx) +
                    "," +
                    xypic.em2px(-p.y - 2 * ny);
                  d2 +=
                    " Q" + xypic.em2px(p.x + nx) + "," + xypic.em2px(-p.y - ny);
                  ...
                  ny = amp * Math.sin(angle + hpi);
                  d1 +=
                    " " + xypic.em2px(p.x + nx) + "," + xypic.em2px(-p.y - ny);
              ...
              case "2":
                ...
                for (var i = 0; i < n; i++) {
                  ...
                  d1 +=
                    " Q" +
                    xypic.em2px(p.x + 3 * nx) +
                    "," +
                    xypic.em2px(-p.y - 3 * ny);
                  d2 +=
                    " Q" + xypic.em2px(p.x + nx) + "," + xypic.em2px(-p.y - ny);
            ...
            switch (variant) {
              case "3":
                ...
                for (var i = 0; i <= n; i++) {
                  ...
                  ny = amp * Math.sin(angle + hpi);
                  d1 +=
                    " M" + xypic.em2px(p.x + nx) + "," + xypic.em2px(-p.y - ny);
                  d2 += " M" + xypic.em2px(p.x) + "," + xypic.em2px(-p.y);
                  d3 +=
                    " M" + xypic.em2px(p.x - nx) + "," + xypic.em2px(-p.y + ny);
          ...
          if (objectForConnect !== undefined) {
            ...
            switch (main) {
                ...
                if (len >= dash) {
                  switch (variant) {
                    ...
                    case "3":
                      ...
                      var shiftLen =
                        (len - dash) / 2 -
                        Math.floor((len - dash) / 2 / dash) * dash;
                      var shiftT = this.tOfLength(shiftLen);
                      var shifted = this.divide(shiftT)[1];
                      shifted.drawPrimitive(
                        svg,
                        xypic.em2px(dash) + " " + xypic.em2px(dash)
                      );
                      break;
              ...
              default:
                ...
                var dummyContext = xypic.DrawingContext(
                  xypic.Shape.none,
                  dummyEnv
                );
            ...
            var shiftLen =
              (len - n * objectLen + objectLen - objectWidth) / 2 +
              objectBBox.l;
            var dummyContext = xypic.DrawingContext(xypic.Shape.none, dummyEnv);
            for (var i = 0; i < n; i++) {
          ...
          if (objectForConnect !== undefined) {
            ...
            switch (main) {
              ...
              default:
                ...
                shape = xypic.Shape.CurveShape(
                  this,
                  objectForDrop,
                  objectForConnect,
                  this.boundingBox(vshift)
                );
    ...
    xypic.Curve.QuadBezier = xypic.Curve.Subclass({
      ...
      tOfIntersections: function (frame) {
        ...
        if (frame.isRect()) {
          ...
          tsCandidate = tsCandidate.concat(
            xypic.Curve.solutionsOfQuadEq(a2x, a1x, a0x - lx)
          );
          ...
          tsCandidate = tsCandidate.concat(
            xypic.Curve.solutionsOfQuadEq(a2y, a1y, a0y - dy)
          );
          for (var i = 0; i < tsCandidate.length; i++) {
            var t = tsCandidate[i];
            var x = px(t);
          ...
          var arc0 = xypic.CurveSegment.Arc(
            cx,
            cy,
            rx,
            ry,
          ...
          var arc1 = xypic.CurveSegment.Arc(
            cx,
            cy,
            rx,
            ry,
          ...
          var arc2 = xypic.CurveSegment.Arc(
            cx,
            cy,
            rx,
            ry,
          ...
          var arc3 = xypic.CurveSegment.Arc(
            cx,
            cy,
            rx,
            ry,
          ...
          var intersec = [];
          intersec = intersec.concat(
            xypic.CurveSegment.findIntersections(arc0, bezier)
          );
          intersec = intersec.concat(
            xypic.CurveSegment.findIntersections(arc1, bezier)
          );
          intersec = intersec.concat(
            xypic.CurveSegment.findIntersections(arc2, bezier)
          );
          intersec = intersec.concat(
            xypic.CurveSegment.findIntersections(arc3, bezier)
          );
    ...
    xypic.Curve.CubicBezier = xypic.Curve.Subclass({
      ...
      tOfIntersections: function (frame) {
        ...
        if (frame.isRect()) {
          ...
          tsCandidate = tsCandidate.concat(
            xypic.Curve.solutionsOfCubicEq(a3x, a2x, a1x, a0x - lx)
          );
          for (var i = 0; i < tsCandidate.length; i++) {
            var t = tsCandidate[i];
            var y = py(t);
          ...
          tsCandidate = tsCandidate.concat(
            xypic.Curve.solutionsOfCubicEq(a3y, a2y, a1y, a0y - dy)
          );
          for (var i = 0; i < tsCandidate.length; i++) {
            var t = tsCandidate[i];
            var x = px(t);
    ...
    xypic.Curve.CubicBeziers = xypic.Curve.Subclass(
      {
        ...
        divide: function (t) {
          ...
          divE.unshift(divB[1]);
          return [
            xypic.Curve.CubicBeziers(divS),
            xypic.Curve.CubicBeziers(divE),
          ];
        },
        slice: function (t0, t1) {
          if (t0 >= t1) {
    ...
    xypic.Curve.Line = MathJax.Object.Subclass({
      ...
      tOfIntersections: function (frame) {
        ...
        if (frame.isRect()) {
          ...
          tsCandidate = tsCandidate.concat(
            xypic.Curve.solutionsOfLinearEq(a1x, a0x - lx)
          );
          for (var i = 0; i < tsCandidate.length; i++) {
            var t = tsCandidate[i];
            var y = py(t);
          ...
          tsCandidate = tsCandidate.concat(
            xypic.Curve.solutionsOfLinearEq(a1y, a0y - dy)
          );
          for (var i = 0; i < tsCandidate.length; i++) {
            var t = tsCandidate[i];
            var x = px(t);
        ...
        if (s.x !== e.x || s.y !== e.y) {
          ...
          switch (main) {
            case "":
              // draw invisible line
              env.angle = angle;
              env.lastCurve = xypic.LastCurve.Line(
                s,
                e,
                env.p,
                env.c,
        ...
        if (s.x !== e.x || s.y !== e.y) {
          ...
          switch (main) {
            ...
            case "==":
              ...
              if (len >= dash) {
                var shiftLen =
                  (len - dash) / 2 - Math.floor((len - dash) / 2 / dash) * dash;
                shift = {
            ...
            case "~":
              ...
              if (len >= wave) {
                ...
                var d =
                  ...
                  xypic.em2px(sy) +
                  " Q" +
                  xypic.em2px(sx + tx + cx) +
                  "," +
                  xypic.em2px(sy - ty - cy) +
                  " " +
                  xypic.em2px(sx + 2 * tx) +
                  "," +
                  xypic.em2px(sy - 2 * ty) +
                ...
                for (var i = 1; i < n; i++) {
                  d +=
                    " T" +
                    xypic.em2px(sx + (4 * i + 2) * tx) +
                    "," +
                    xypic.em2px(sy - (4 * i + 2) * ty) +
                    " T" +
                    xypic.em2px(sx + (4 * i + 4) * tx) +
                    "," +
            ...
            case "~~":
              ...
              if (len >= wave) {
                ...
                for (var i = 0; i <= n; i++) {
                  d +=
                    " M" +
                    xypic.em2px(sx + 8 * i * tx) +
                    "," +
                    xypic.em2px(sy - 8 * i * ty) +
                    " Q" +
                    xypic.em2px(sx + (8 * i + 1) * tx + cx) +
                    "," +
                    xypic.em2px(sy - (8 * i + 1) * ty - cy) +
                    " " +
                    xypic.em2px(sx + (8 * i + 2) * tx) +
                    "," +
                    xypic.em2px(sy - (8 * i + 2) * ty) +
                    " T" +
                    xypic.em2px(sx + (8 * i + 4) * tx) +
                    "," +
      ...
      drawSquigglyLineShape: function (svg, d, s, e, cx, cy, variant) {
        ...
        if (variant === "3") {
          ...
          g1 = svg.createGroup(
            svg.transformBuilder().translate(cx / 2, cy / 2)
          );
          g1.createSVGElement("path", { d: d });
          g2 = svg.createGroup(
            svg.transformBuilder().translate(-cx / 2, -cy / 2)
    ...
    xypic.CurveSegment = MathJax.Object.Subclass(
      {
        ...
        clippedLineRange: function (ps, lineMin, lineMax) {
          ...
          for (var i = 0; i <= n; i++) {
            es[i] = [
              i / n,
              -lineMin[0] * ps[i].x - lineMin[1] * ps[i].y - lineMin[2],
              1,
            ];
          }
          ...
          if (es[0][1] < 0) {
            ...
            for (i = 1; i <= n; i++) {
              ...
              if (
                v > 0 &&
                v < 1 &&
                (vminAgainstLineMin === undefined || v < vminAgainstLineMin)
              ) {
          ...
          if (es[n][1] < 0) {
            for (i = 0; i < n; i++) {
              ...
              if (
                v > 0 &&
                v < 1 &&
                (vmaxAgainstLineMin === undefined || v > vmaxAgainstLineMin)
              ) {
          ...
          for (i = 0; i <= n; i++) {
            es[i] = [
              i / n,
              lineMax[0] * ps[i].x + lineMax[1] * ps[i].y + lineMax[2],
              1,
            ];
          }
          ...
          if (es[0][1] < 0) {
            ...
            for (i = 1; i <= n; i++) {
              ...
              if (
                v > 0 &&
                v < 1 &&
                (vminAgainstLineMax === undefined || v < vminAgainstLineMax)
              ) {
          ...
          if (es[n][1] < 0) {
            for (i = 0; i < n; i++) {
              ...
              if (
                v > 0 &&
                v < 1 &&
                (vmaxAgainstLineMax === undefined || v > vmaxAgainstLineMax)
              ) {
        ...
        findIntersections: function (segment0, segment1) {
          ...
          while (i < n && queue.length > 0) {
            ...
            if (tlen < acc && segment0.paramLength() < acc) {
              ...
              if (swapped) {
                intersections.push([
                  segment1.clip(tmin, tmax).paramRange(),
                  segment0.paramRange(),
                ]);
              } else {
                intersections.push([
            ...
            if (tlen <= segment1.paramLength() * 0.8) {
              ...
              if (tlen > segment0.paramLength()) {
                ...
                queue.push([
                  newSegment,
                  segment0.clip(range0.min, mid0),
                  !swapped,
                ]);
                queue.push([
                  newSegment,
                  segment0.clip(mid0, range0.max),
                  !swapped,
                ]);
              }
            }
    ...
    xypic.CurveSegment.QuadBezier = xypic.CurveSegment.Subclass({
      Init: function (bezier, tmin, tmax) {
        ...
        this.cp1 = xypic.Frame.Point(
          (1 - tmax) * (1 - tmin) * bezier.cp0.x +
            (tmin + tmax - 2 * tmin * tmax) * bezier.cp1.x +
            tmin * tmax * bezier.cp2.x,
          (1 - tmax) * (1 - tmin) * bezier.cp0.y +
            (tmin + tmax - 2 * tmin * tmax) * bezier.cp1.y +
            tmin * tmax * bezier.cp2.y
        );
    ...
    xypic.CurveSegment.CubicBezier = xypic.CurveSegment.Subclass({
      Init: function (bezier, tmin, tmax) {
        ...
        this.cp0 = bezier.position(tmin);
        this.cp1 = xypic.Frame.Point(
          (1 - tmax) * (1 - tmin) * (1 - tmin) * bezier.cp0.x +
            (1 - tmin) * (2 * tmin + tmax - 3 * tmin * tmax) * bezier.cp1.x +
            tmin * (2 * tmax + tmin - 3 * tmin * tmax) * bezier.cp2.x +
            tmin * tmin * tmax * bezier.cp3.x,
          (1 - tmax) * (1 - tmin) * (1 - tmin) * bezier.cp0.y +
            (1 - tmin) * (2 * tmin + tmax - 3 * tmin * tmax) * bezier.cp1.y +
            tmin * (2 * tmax + tmin - 3 * tmin * tmax) * bezier.cp2.y +
            tmin * tmin * tmax * bezier.cp3.y
        );
        this.cp2 = xypic.Frame.Point(
          (1 - tmin) * (1 - tmax) * (1 - tmax) * bezier.cp0.x +
            (1 - tmax) * (2 * tmax + tmin - 3 * tmin * tmax) * bezier.cp1.x +
            tmax * (2 * tmin + tmax - 3 * tmin * tmax) * bezier.cp2.x +
            tmin * tmax * tmax * bezier.cp3.x,
          (1 - tmin) * (1 - tmax) * (1 - tmax) * bezier.cp0.y +
            (1 - tmax) * (2 * tmax + tmin - 3 * tmin * tmax) * bezier.cp1.y +
            tmax * (2 * tmin + tmax - 3 * tmin * tmax) * bezier.cp2.y +
            tmin * tmax * tmax * bezier.cp3.y
        );
    ...
    xypic.CurveSegment.Arc = xypic.CurveSegment.Subclass({
      ...
      fatLine: function () {
        ...
        if (r < AST.xypic.machinePrecision) {
          var Lmin = [
            1,
            0,
            this.x * ry * cosp + this.y * rx * sinp + rx * ry * Math.cos(tm),
          ];
          ...
          var Lmax = [
            -rry * cosp,
            -rrx * sinp,
            this.x * rry * cosp + this.y * rrx * sinp + (rx * ry) / r,
          ];
        }
        return { min: Lmin, max: Lmax };
      },
      clip: function (angleMin, angleMax) {
        return xypic.CurveSegment.Arc(
          this.x,
          this.y,
          this.rx,
          this.ry,
      ...
      clippedRange: function (origLineMin, origLineMax) {
        ...
        var d0 = -(
          lineMin[0] * (x + r * Math.cos(angleMin)) +
          lineMin[1] * (y + r * Math.sin(angleMin)) +
          lineMin[2]
        );
        var d1 = -(
          lineMin[0] * (x + r * Math.cos(angleMax)) +
          lineMin[1] * (y + r * Math.sin(angleMax)) +
          lineMin[2]
        );
        ...
        var d0 =
          lineMax[0] * (x + r * Math.cos(angleMin)) +
          lineMax[1] * (y + r * Math.sin(angleMin)) +
          lineMax[2];
        var d1 =
          lineMax[0] * (x + r * Math.cos(angleMax)) +
          lineMax[1] * (y + r * Math.sin(angleMax)) +
          lineMax[2];
    ...
    xypic.LastCurve.Line = xypic.LastCurve.Subclass({
      ...
      derivative: function (t) {
        return xypic.Frame.Point(this.c.x - this.p.x, this.c.y - this.p.y);
      },
      angle: function (t) {
    ...
    xypic.LastCurve.CubicBSpline = xypic.LastCurve.Subclass({
      ...
      segments: function () {
        ...
        for (var i = 0; i < n; i++) {
          segments[i] = xypic.CurveSegment.CubicBezier(
            this.origBezier,
            i / n,
            (i + 1) / n
          );
    ...
    AST.Place.Intercept.Augment({
      value: function (context) {
        ...
        if (!tmpEnv.lastCurve.isDefined) {
          tmpEnv.lastCurve = xypic.LastCurve.Line(
            tmpEnv.p,
            tmpEnv.c,
            tmpEnv.p,
            tmpEnv.c,
        ...
        for (var i = 0; i < thisSegs.length; i++) {
          for (var j = 0; j < thatSegs.length; j++) {
            intersec = intersec.concat(
              xypic.CurveSegment.findIntersections(thisSegs[i], thatSegs[j])
            );
          }
        }
        ...
        if (intersec.length === 0) {
          ...
          while (!stop && k < n) {
            ...
            do {
              ...
              if (
                d0 * d0 + d1 * d1 <
                goalAccuracy * goalAccuracy * (x0 * x0 + x1 * x1)
              ) {
                stop = true;
              } else {
                var newX0 = x0 - d0;
                ...
                var rho =
                  (f0 * f0 + f1 * f1 - (newF0 * newF0 + newF1 * newF1)) /
                  (d0 * (mu * d0 + g0) + d1 * (mu * d1 + g1));
    ...
    AST.Pos.SaveBase.Augment({
      toShape: function (context) {
        ...
        env.savePos(
          this.id,
          xypic.Saving.Base(env.origin, env.xBase, env.yBase)
        );
      },
    });
    ...
    AST.Object.Augment({
      toDropShape: function (context) {
        ...
        if (modifiers.isEmpty) {
          ...
          objectShape = modifiers.head.modifyShape(
            subcontext,
            objectShape,
            modifiers.tail
          );
    ...
    AST.ObjectBox.Augment({
      toConnectShape: function (context) {
        ...
        if (s.x !== e.x || s.y !== e.y) {
          var shape = xypic.Curve.Line(s, e).toShape(
            context,
            this,
    ...
    AST.ObjectBox.Text.Augment({
      toDropShape: function (context) {
        var env = context.env;
        var textShape = xypic.Shape.TextShape(
          env.c,
          this.math,
          xypic.svgForTestLayout
        );
    ...
    AST.ObjectBox.Cir.Cir.Segment.Augment({
      toDropShape: function (context, x, y, r) {
        ...
        var circleSegmentShape = xypic.Shape.CircleSegmentShape(
          x,
          y,
          sx,
          sy,
        ...
        switch (this.main) {
          case "--":
            ...
            if (convertToEllipse) {
              ...
              shape = xypic.Shape.EllipseShape(
                x + (right - left) / 2,
                y + (up - down) / 2,
                xy.x,
                xy.y,
              ...
              shape = xypic.Shape.RectangleShape(
                x,
                y,
                left,
                right,
          ...
          case ",":
            var depth = this.radius.depth(context);
            shape = xypic.Shape.BoxShadeShape(
              x,
              y,
              left,
              right,
          ...
          case "(":
            shape = xypic.Shape.LeftParenthesis(
              x - left,
              y + (up - down) / 2,
              up + down,
              0,
        ...
        switch (this.main) {
          ...
          default:
            ...
            if (newdirObj !== undefined) {
              ...
              throw xypic.ExecutionError(
                "\\dir " + this.variant + "{" + this.main + "} not defined."
              );
            }
        }
      ...
      toConnectShape: function (context) {
        ...
        switch (controlPoints.length) {
          ...
          case 1:
            ...
            if (
              tOfShavedStart === undefined ||
              tOfShavedEnd === undefined ||
              tOfShavedStart >= tOfShavedEnd
            ) {
              env.angle = 0;
              env.lastCurve = xypic.LastCurve.none;
              return xypic.Shape.none;
            }
            shape = origBezier.toShape(
              context,
              objectForDrop,
              objectForConnect
            );
            env.lastCurve = xypic.LastCurve.QuadBezier(
              origBezier,
              tOfShavedStart,
              tOfShavedEnd,
              shape
          ...
          case 2:
            var origBezier = xypic.Curve.CubicBezier(
              s,
              controlPoints[0],
              controlPoints[1],
              e
            ...
            env.lastCurve = xypic.LastCurve.CubicBezier(
              origBezier,
              tOfShavedStart,
              tOfShavedEnd,
              shape
          ...
          default:
            ...
            shape = origBeziers.toShape(
              context,
              objectForDrop,
              objectForConnect
            );
            env.lastCurve = xypic.LastCurve.CubicBSpline(
              s,
              e,
              origBeziers,
              tOfShavedStart,
    ...
    AST.Coord.DeltaRowColumn.Augment({
      position: function (context) {
        ...
        if (row === undefined || col === undefined) {
          throw xypic.ExecutionError(
            "xymatrix rows and columns not found for " + this.toSring()
          );
        }
    ...
    AST.Modifier.Augment({
      proceedModifyShape: function (context, objectShape, restModifiers) {
        if (restModifiers.isEmpty) {
          ...
          return restModifiers.head.modifyShape(
            context,
            objectShape,
            restModifiers.tail
          );
    ...
    AST.Modifier.Shape.Alphabets.Augment({
      preprocess: function (context, reversedProcessedModifiers) {
        var modifier = xypic.repositories.modifierRepository.get(
          this.alphabets
        );
        if (modifier !== undefined) {
    ...
    AST.Modifier.Shape.DefineShape.Augment({
      preprocess: function (context, reversedProcessedModifiers) {
        ...
        xypic.repositories.modifierRepository.put(
          this.shape,
          AST.Modifier.Shape.CompositeModifiers(processedModifiers)
        );
      },
      modifyShape: function (context, objectShape, restModifiers) {
        return this.proceedModifyShape(context, objectShape, restModifiers);
    ...
    AST.Modifier.AddOp.Augment({
      ...
      modifyShape: function (context, objectShape, restModifiers) {
        ...
        context.appendShapeToFront(
          xypic.Shape.InvisibleBoxShape(context.env.c)
        );
        return this.proceedModifyShape(context, objectShape, restModifiers);
      },
    });
    ...
    AST.Modifier.Shape.Frame.Augment({
      ...
      modifyShape: function (context, objectShape, restModifiers) {
        ...
        if (env.c !== undefined) {
          ...
          var frameShape = frameObject.toDropFilledShape(
            dummyContext,
            colorName,
            env.c.isCircle()
          );
                        ...
                        .append(
                          AST.ObjectBox.Curve.PosList.Pos(
                            AST.Pos.Coord(
                              AST.Coord.C(),
                              FP.List.empty
                                .append(AST.Pos.SwapPAndC(AST.Coord.C()))
                                .append(
                                  AST.Pos.Plus(
                                    AST.Coord.Vector(
                                      AST.Vector.Dir(this.outDirection, "3pc")
                                    )
                                  )
                                )
                            )
                            ...
                            AST.Pos.Coord(
                              ...
                              FP.List.empty
                                ...
                                .append(
                                  AST.Pos.Plus(
                                    AST.Coord.Vector(
                                      AST.Vector.Dir(this.inDirection, "3pc")
                                    )
                                  )
                                )
                            )
    ...
    AST.Command.Ar.Form.Slide.Augment({
      toShape: function (context) {
        context.env.segmentSlideEm = FP.Option.Some(
          xypic.length2em(this.slideDimen)
        );
      },
    });
    ...
    AST.Command.Ar.Form.Conn.Object.Augment({
      getObject: function (context) {
        var modifiers = context.env.arrowObjectModifiers.concat(
    ...
    AST.Command.Path.TurningSegment.Augment({
      toShape: function (context) {
        ...
        var deo = circle.relativeEndPoint(
          context,
          r + (circle.orient === "^" ? slideEm : -slideEm)
        );
    ...
    AST.ObjectBox.Cir.Cir.Segment.Augment({
      ...
      relativeEndPoint: function (context, r) {
        var angle;
        angle =
          (this.endPointDegree(context, this.relativeStartPointAngle(context)) /
            180) *
          Math.PI;
        return { x: r * Math.cos(angle), y: r * Math.sin(angle) };
      },
    ...
    AST.Command.Path.Turn.Diag.Augment({
      explicitizedCircle: function (context) {
        ...
        var det =
          (env.c.x - env.p.x) * Math.sin(angle) -
          (env.c.y - env.p.y) * Math.cos(angle);
        orient = det < 0 ? "^" : "_";
        endDiag = startDiag.turn(orient);
        return AST.ObjectBox.Cir.Cir.Segment(startDiag, orient, endDiag);
      },
    ...
    AST.Command.Path.Segment.Augment({
      setupPositions: function (context) {
        ...
        if (slideEm !== 0) {
          p = p.move(
            p.x + slideEm * Math.cos(angle),
            p.y + slideEm * Math.sin(angle)
          );
          c = c.move(
            c.x + slideEm * Math.cos(angle),
            c.y + slideEm * Math.sin(angle)
          );
        }
      ...
      toShape: function (context) {
        ...
        if (labelmargin !== 0) {
          ...
          if (bbox !== undefined) {
            ...
            context.appendShapeToFront(
              xypic.Shape.TranslateShape(margin * cos, margin * sin, labelShape)
            );
          }
        } else {
          ...
          this.rows.map(function (row) {
            ...
            var rowModel = xypic.Xymatrix.Row(
              row.entries.map(function (entry) {
                ...
                var localContext = xypic.DrawingContext(
                  xypic.Shape.none,
                  localEnv
                );
      ...
      */
        ...
        matrix.rows.foreach(function (row) {
          ...
          row.entries.foreach(function (entry) {
            ...
            var colnum = colIndex + 1;
            var rownum = rowIndex + 1;
            var objectShape = xypic.Shape.TranslateShape(
              x,
              y,
              entry.objectShape
            );
    ...
    xypic.Xymatrix.Entry = MathJax.Object.Subclass({
      ...
      getDistanceToEdgePoint: function (frame, angle) {
        var edgePoint = frame.edgePoint(
          frame.x + Math.cos(angle),
          frame.y + Math.sin(angle)
        );
      ...
      getU: function (orientation) {
        ...
        return this.getDistanceToEdgePoint(
          this.frame,
    ...
    AST.Command.Xymatrix.Setup.ChangeSpacing.Row.Augment({
      toShape: function (context) {
        var env = context.env;
        env.xymatrixRowSepEm = this.addop.applyToDimen(
          env.xymatrixRowSepEm,
    ...
    AST.Command.Xymatrix.Setup.ChangeSpacing.Column.Augment({
      toShape: function (context) {
        var env = context.env;
        env.xymatrixColSepEm = this.addop.applyToDimen(
          env.xymatrixColSepEm,
          xypic.length2em(this.dimen)
        );
      },
    });
    ...
    AST.Command.Xymatrix.Setup.ChangeSpacing.RowAndColumn.Augment({
      toShape: function (context) {
        var env = context.env;
        var sepEm = this.addop.applyToDimen(
          env.xymatrixRowSepEm,
          xypic.length2em(this.dimen)
        );
    ...
    AST.Command.Xymatrix.Setup.PretendEntrySize.Height.Augment({
      toShape: function (context) {
        context.env.xymatrixPretendEntryHeight = FP.Option.Some(
          xypic.length2em(this.dimen)
        );
      },
    });
    ...
    AST.Command.Xymatrix.Setup.PretendEntrySize.Width.Augment({
      toShape: function (context) {
        context.env.xymatrixPretendEntryWidth = FP.Option.Some(
          xypic.length2em(this.dimen)
        );
      },
    });
    ...
    AST.Command.Xymatrix.Setup.AdjustEntrySize.Margin.Augment({
      toShape: function (context) {
        var env = context.env;
        env.objectmargin = this.addop.applyToDimen(
          env.objectmargin,
          xypic.length2em(this.dimen)
        );
      },
    ...
    AST.Command.Xymatrix.Setup.AdjustEntrySize.Width.Augment({
      toShape: function (context) {
        var env = context.env;
        env.objectwidth = this.addop.applyToDimen(
          env.objectwidth,
          xypic.length2em(this.dimen)
        );
      },
    ...
    AST.Command.Xymatrix.Setup.AdjustEntrySize.Height.Augment({
      toShape: function (context) {
        var env = context.env;
        env.objectheight = this.addop.applyToDimen(
          env.objectheight,
          xypic.length2em(this.dimen)
        );
      },
    ...
    AST.Command.Xymatrix.Setup.AdjustLabelSep.Augment({
      toShape: function (context) {
        var env = context.env;
        env.labelmargin = this.addop.applyToDimen(
          env.labelmargin,
          xypic.length2em(this.dimen)
        );
      },
    ...
    AST.Command.Xymatrix.Entry.SimpleEntry.Augment({
      toShape: function (context) {
        ...
        var defaultHeight = xypic.em2length(
          env.objectmargin + env.objectheight
        );
        var defaultSizeModifier = AST.Modifier.AddOp(
          AST.Modifier.AddOp.GrowTo(),
          AST.Modifier.AddOp.VactorSize(
            AST.Vector.Abs(defaultWidth, defaultHeight)
          )
        );
        var margin = xypic.em2length(env.objectmargin);
        var marginModifier = AST.Modifier.AddOp(
          AST.Modifier.AddOp.Grow(),
          AST.Modifier.AddOp.VactorSize(AST.Vector.Abs(margin, margin))
        );
        var modifiers = this.modifiers
          .concat(env.xymatrixEntryModifiers)
          .prepend(defaultSizeModifier)
    ...
    AST.Command.Xymatrix.Entry.EmptyEntry.Augment({
      toShape: function (context) {
        ...
        var modifiers = env.xymatrixEntryModifiers
          .prepend(defaultSizeModifier)
          .prepend(marginModifier);
    ...
    AST.Command.Twocell.Augment({
      toShape: function (context) {
        ...
        var subEnv = origEnv.duplicate();
        var subcontext = xypic.DrawingContext(xypic.Shape.none, subEnv);
        subEnv.twocellmodmapobject =
          origEnv.twocellmodmapobject ||
          AST.Object(FP.List.empty, AST.ObjectBox.Dir("", "|"));
        subEnv.twocellhead =
          origEnv.twocellhead ||
          AST.Object(FP.List.empty, AST.ObjectBox.Dir("", ">"));
        subEnv.twocelltail =
          origEnv.twocelltail ||
          AST.Object(FP.List.empty, AST.ObjectBox.Dir("", ""));
        subEnv.twocellarrowobject =
          origEnv.twocellarrowobject ||
          AST.Object(FP.List.empty, AST.ObjectBox.Dir("", "=>"));

        subEnv.twocellUpperCurveObjectSpacer =
          origEnv.twocellUpperCurveObjectSpacer;
        subEnv.twocellUpperCurveObject = origEnv.twocellUpperCurveObject;
        subEnv.twocellLowerCurveObjectSpacer =
          origEnv.twocellLowerCurveObjectSpacer;
    ...
    AST.Command.Twocell.Hops2cell.Augment({
      toShape: function (context, arrow) {
        ...
        var m = xypic.Frame.Point(s.x + dx * 0.5, s.y + dy * 0.5);
        var tangle = Math.atan2(dy, dx);
        var antiClockwiseAngle = tangle + Math.PI / 2;

        var curvatureEm = env.twocellCurvatureEm.getOrElse(
          this.getDefaultCurvature()
        );
        ...
        if (env.twocellShouldDrawCurve) {
          ...
          if (env.lastCurve.isDefined) {
            env.angle = tangle;
            var ucmp = this.getUpperLabelPosition(
              s,
              e,
              m,
              curvatureEm,
            ...
            var uangle = this.getUpperLabelAngle(
              antiClockwiseAngle,
              s,
              e,
              m,
          ...
          if (env.lastCurve.isDefined) {
            env.angle = tangle;
            var lcmp = this.getLowerLabelPosition(
              s,
              e,
              m,
              curvatureEm,
            ...
            var langle = this.getLowerLabelAngle(
              antiClockwiseAngle,
              s,
              e,
              m,
      ...
      _toCurveShape: function (
        ...
        var curveShape = origBezier.toShape(
          context,
          objectForDrop,
          objectForConnect
        );
    ...
    AST.Command.Twocell.Twocell.Augment({
      ...
      getUpperLabelPosition: function (
        s,
        e,
        midPoint,
        curvatureEm,
      ...
      getLowerLabelPosition: function (
        s,
        e,
        midPoint,
        curvatureEm,
    ...
    AST.Command.Twocell.UpperTwocell.Augment({
      ...
      toLowerCurveShape: function (
        ...
        if (shavedS.x !== shavedE.x || shavedS.y !== shavedE.y) {
          context.env.lastCurve = xypic.LastCurve.Line(
            shavedS,
            shavedE,
            s,
            e,
    ...
    AST.Command.Twocell.CompositeMap.Augment({
      ...
      getUpperLabelPosition: function (
        ...
        var l = Math.sqrt(dx * dx + dy * dy);
        return xypic.Frame.Point(e.x + 0.5 * dx, e.y + 0.5 * dy);
      },
      getLowerLabelPosition: function (
        s,
        e,
        ...
        var l = Math.sqrt(dx * dx + dy * dy);
        return xypic.Frame.Point(s.x + 0.5 * dx, s.y + 0.5 * dy);
      },
      getUpperLabelAngle: function (
        antiClockwiseAngle,
        s,
    ...
    AST.Command.Twocell.Switch.SetCurvature.Augment({
      setup: function (context) {
        ...
        if (this.nudge.isOmit) {
          ...
          env.twocellCurvatureEm = FP.Option.Some(
            this.nudge.number * AST.xypic.lineElementLength
          );
        }
      },
    });
    ...
    AST.Command.Twocell.Arrow.WithOrientation.Augment({
      ...
      toArrowShape: function (context, labelOrigin) {
        ...
        switch (this.tok) {
          ...
          case "=":
            var angle = env.angle;
            var shape = xypic.Shape.TwocellEqualityArrowheadShape(
              env.c,
              env.angle
            );
            context.appendShapeToFront(shape);
    ...
    AST.Command.Twocell.Arrow.WithPosition.Augment({
      ...
      toArrowShape: function (context, labelOrigin) {
        ...
        if (!nudge.isOmit) {
          env.c = xypic.Frame.Point(
            arrowPos.x +
              AST.xypic.lineElementLength * Math.cos(angle + Math.PI / 2),
            arrowPos.y +
              AST.xypic.lineElementLength * Math.sin(angle + Math.PI / 2)
          );
          this.labelObject.toDropShape(context);
        }
        env.c = c;
    ...
    AST.Pos.Xyimport.Graphics.Augment({
      toShape: function (context) {
        ...
        if (
          !subEnv.includegraphicsWidth.isDefined ||
          !subEnv.includegraphicsHeight.isDefined
        ) {
          throw xypic.ExecutionError(
...
MathJax.Hub.Register.StartupHook("HTML-CSS Xy-pic Require", function () {
  ...
  var setupHTMLCSSMeasure = function () {
    ...
    AST.xypic.dottedDasharray =
      "" + xypic.oneem + " " + xypic.em2px(AST.xypic.thickness);
  };
        ...
        _draw: function (svg, test) {
          ...
          var mathSpan = HTMLCSS.Element("span", {
            ...
            style: {
              ...
              "aria-readonly": "true",
              position: "absolute",
              color: svg.getCurrentColor() /*, "border":"0.1px dashed" */,
            },
          });
          ...
          if (!test) {
            ...
            mathSpan.setAttribute(
              "y",
              -c.y - halfHD - origin.y - stack.offsetTop / HTMLCSS.em + H
            );
      ...
      if (xypicData) {
        ...
        if (box !== undefined) {
          ...
          svg.setAttribute(
            "viewBox",
            [
              xypic.em2px(box.x - box.l - p),
              -xypic.em2px(box.y + box.u + p),
              xypic.em2px(box.l + box.r + 2 * p),
              xypic.em2px(box.u + box.d + 2 * p),
            ].join(" ")
          );
          ...
          for (var i = 0; i < c; i++) {
            ...
            to.style.left =
              "" + xypic.em2px(x + box.l + p * xypic.oneem) + "px";
  ...
  AST.xypic.newdir.Augment({
    toHTML: function (span) {
      var newdir = this.cmd;
      xypic.repositories.dirRepository.put(
        newdir.dirMain,
...
MathJax.Hub.Register.StartupHook("SVG Xy-pic Require", function () {
  ...
  var setupSVGMeasure = function (mu, scale) {
    ...
    AST.xypic.axisHeightLength =
      (xypic.axis_height * xypic.length2em("1em")) / 1000;
    ...
    toSVG: function (HW, DD) {
          ...
          if (!test) {
            ...
            var localSVG = BBOX.G({
              ...
              transform:
                "scale(" +
                SVG.em +
                ") matrix(1 0 0 -1 0 0) translate(" +
                xypic.em2px(x / SVG.em) +
                ", " +
                xypic.em2px((c.y - (H - D) / 2) / SVG.em) +
                ")",
      ...
      if (xypicData) {
        ...
        if (shapeBBox !== undefined) {
          ...
          xypicBBOX.element.setAttribute(
            "transform",
            "scale(" +
              1 / SVG.em +
              ") matrix(1 0 0 -1 0 0) translate(0," +
