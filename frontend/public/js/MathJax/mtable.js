...
MathJax.Hub.Register.StartupHook("HTML-CSS Jax Ready", function () {
  ...
  MML.mtable.Augment({
    toHTML: function (span) {
      ...
      if (values.equalrows) {
        ...
        var Hm = Math.max.apply(Math, H),
          Dm = Math.max.apply(Math, D);
        for (i = 0, m = A.length; i < m; i++) {
          s = (Hm + Dm - (H[i] + D[i])) / 2;
          H[i] += s;
          D[i] += s;
      ...
      for (i = 0, m = A.length - 1; i < m; i++) {
        ...
        if (RLINES[i] !== MML.LINES.NONE && RLINES[i] !== "") {
          ...
          HTMLCSS.placeBox(
            line,
            0,
            y - D[i] - (dy - D[i] - H[i + 1]) / 2,
            true
          );
          if (RLINES[i] === MML.LINES.DASHED)
      ...
      if (C[LABEL]) {
        ...
        var labelspace = HTMLCSS.length2em(
          values.minlabelspacing,
          mu,
          HTMLCSS.cwidth
        );
        var labelW = labelspace + C[LABEL].bbox.w,
          labelshift = 0,
          tw = mw;
        ...
        C[LABEL].style[s === 1 ? "marginLeft" : "marginRight"] = HTMLCSS.Em(
          s * labelshift
        );
        span.bbox.tw = tw;
    ...
    HTMLhandleSpace: function (span) {
      ...
      if (!this.hasFrame && span.bbox.width == null) {
        span.firstChild.style.marginLeft = span.firstChild.style.marginRight =
          HTMLCSS.Em(1 / 6);
        span.bbox.w += 1 / 3;
        span.bbox.rw += 1 / 3;
        span.bbox.lw += 1 / 6;
  ...
  MathJax.Ajax.loadComplete("[Extra]/mtable.js");
});
